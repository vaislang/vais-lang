...
> iteration: 33 (cap exceeded)
> max_iterations: 30
>   strategy: sequential. A1/A3/A2/B1/B2/B3/B4/B5/C1/C2/C3/D/E1-E7 ✓. Phase F (TC span-bleed 완전 차단) / Phase G (aggregate coerce 일반화) 신설 권장. 세션 commit 33+건, regression 0. test_types 링크+실행 유지.
...
- [ ] Phase E. Per-test codegen bugs (14 vaisdb tests) [blockedBy: (없음)]
  ...
  더 이상의 세분화 수정은 ROI 낮음. 다음 세션 권장 접근:
    1. Phase F (신설): TC expr_types 재생성 — cross-module span bleed 완전 차단 (spans include file_id)
    2. Phase G: aggregate 값/포인터 자동 변환 (bitcast/alloca+store+load)을 모든 arg/ret 경계에 적용하는 일반화된 coercion pass
>
