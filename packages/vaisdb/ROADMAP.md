...
| 16 | Runtime Pipeline & Compiler Gap Closure | ⏸ Paused | 3/8 (A1/A2/A2.5 완료, 다음 세션에서 A3/A5/A6/A7/A4) |
...
> mode: paused (2026-04-21 — A1/A2/A2.5 커밋 후 범위 경계에서 중단. 재개 시 /harness)
> iteration: 1
>
> ### 세션 1 (2026-04-21) 요약
...
- [ ] A3. compute_sizeof enum struct variant path 구현 [blockedBy: A1]
  scope: Vec/Array ops에서 enum struct variant (FloatVal 등) 원소 크기 fallback 8 bytes → 정확 크기 계산
  symptom: `[vais-codegen] compute_sizeof: unknown Named type 'FloatVal'/'IntVal'/'StringVal'/...` (test_types 빌드 시 수십 건)
  fix plan: crates/vais-codegen에 enum struct variant lookup 추가 — 부모 enum을 찾아 tagged union payload 크기 반환
  verify: 경고 소거 + vaisdb 15/15 유지
- [ ] A5. enum method receiver ABI (%struct* vs i64) 근본 수정 [blockedBy: A2.5]
  discovered: A2.5 커밋 후 `vaisc run tests/sql/test_types.vais` 시 `test_types.ll:587`에서 발생.
  symptom: `%t2 = call i64 @type_id(i64 %t0)` — `%t0 = alloca %SqlType` (struct pointer) 인데 callsite가 i64 인자로 전달. enum static method dispatch에서 receiver 타입 coercion이 struct pointer → i64 ptrtoint 없이 그냥 쓴 것으로 추정.
  scope: crates/vais-codegen — enum/struct method call의 self/receiver ABI 조사. Int-slot erasure가 필요한 경로 특정.
  verify: test_types 링크 성공 + cargo test 0 regression
- [ ] A6. runtime helpers 대거 구현 [blockedBy: A1]
...
- [ ] A7. cross-module shallow-free emission [blockedBy: A1]
...
- [ ] A4. 링크 파이프라인 end-to-end 검증 [blockedBy: A3, A5, A6, A7]
  verify: vaisc run tests/sql/test_types.vais exit 0 + pass count > 0

진행률: 3/8 (A1, A2, A2.5 완료)
