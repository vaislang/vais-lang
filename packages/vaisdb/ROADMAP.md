...
> **Last Updated**: 2026-05-02 (Core gate green + VaisDB runtime smoke 28/28 + server runtime smoke 3/3)

---

## Compiler Gate Quarantine (2026-04-29)

vaisdb는 compiler Core 안정화 이후 재승격되는 downstream package gate다.
`vaisdb_files` pass count는 계속 측정하지만, 다음 세 gate가 먼저 통과해야
compiler correctness 진척으로 해석할 수 있다:

1. `cd compiler && bash scripts/core-certify.sh`
2. `cd compiler && cargo test -p vais-mir --test lower_strict_tests --release && cargo test -p vais-mir --test core_strict_fixtures --release && cargo test -p vais-mir --test interpreter_tests --release && cargo test -p vais-mir --test validation_tests --release`
3. `cd compiler && cargo test -p vais-codegen --test ret_invariant_test --release && cargo test -p vais-codegen --test index_invariant_test --release && cargo test -p vais-codegen --test call_arg_invariant_test --release`

통합 확인은 `cd compiler && bash scripts/check-integrity.sh`를 사용한다. 최종
출력의 `CORE`, `MIR`, `CODEGEN`, `ECOSYSTEM`, `BACKEND`, `VAISDB RUNTIME`, `SERVER RUNTIME`
항목을 분리해서 본다.

---

## Current Reactivation Baseline (2026-05-01)

검증 기준:
- `cd compiler && bash scripts/check-integrity.sh` 통과.
- 요약: `CORE_CERTIFICATION pass=16 fail=0 total=16`, `MIR OK`, `CODEGEN OK`, `std=82/82`, `vaisdb=261/261`, `phase158=18/18`, `VAISDB RUNTIME smoke=28/28`, `SERVER RUNTIME smoke=3/3`.

현재 실패 집합은 과거 42/41-failure 기준이 아니라 0개 파일이다.

| Primary | Files | 판단 |
|---|---:|---|
| none | 0 | 모든 `vaisdb/src/**/*.vais` 파일이 현재 compiler gate에서 codegen 통과 |
...
The entries below are historical incremental notes. Their intermediate counts
such as `236/261` or `smoke=18/18` explain how the current state was reached;
they are not the active baseline. The active baseline is the table above:
`vaisdb=261/261` and `VAISDB RUNTIME smoke=28/28`.

- `src/vector/search.vais`: `Row.new()`를 잘못 빈 인자로 호출하고 존재하지 않는 `add_column`을 사용하던 코드를 `Vec<SqlValue>` 구성 후 `Row.new(values)` 호출로 정리했다.
- 단독 검증: `vector/search.vais` codegen 통과.
- 최종 `check-integrity.sh`에서는 `vector/search.vais`가 fail list에서 제거됐고 `vaisdb=236/261`로 상승했다.
- compiler package/import type propagation: `rag/chunking/graph.vais`, `security/policy.vais`, `security/mod.vais`, `security/rls.vais`의 C003 indexed collection element type collapse가 해소됐다. 원인은 sequential typecheck와 per-module codegen 사이의 file_id key 불일치였고, compiler build pipeline에서 `modules_map` 기준 AST span file_id를 스탬핑하도록 수정했다.
- 최종 `check-integrity.sh`에서는 C003 계열 4개 파일이 fail list에서 제거됐고 `vaisdb=240/261`로 상승했다.
- `src/fulltext/mod.vais`: `FullTextLock.write_lock`, `FullTextMatchExecutor.execute_search`, `PhraseSearcher.search_phrase`, `PostingListCompactor` 호출부를 현재 fulltext API 정의에 맞춰 정리했다. `fulltext/mod.vais` 단독 codegen이 통과했고 최종 `check-integrity.sh`에서 fail list에서 제거되어 `vaisdb=241/261`로 상승했다.
- `src/sql/catalog/manager.vais`: `BufferPool.write_page` 호출부를 현재 1-인자 API에 맞췄고, startup/delete 경로의 강제 unwrap을 totality-safe match로 바꿨다. `load_from_disk`의 statement-style `M tag` arm들이 서로 다른 cache insert/push 반환 타입을 노출하지 않도록 반환값을 명시적으로 무시했다. `sql/catalog/manager.vais`와 이를 import하던 `sql/executor/mod.vais` 단독 codegen이 모두 통과했고 최종 `check-integrity.sh`에서 두 파일이 fail list에서 제거되어 `vaisdb=243/261`로 상승했다.
- `src/sql/executor/dml.vais`: `get_columns() -> Vec<&ColumnInfo>` 결과를 다시 `&columns`로 순회해 생긴 `&&ColumnInfo` field access를 index-based loop로 정리했다. DML heap write 경로의 `BufferPool.write_page` 호출부를 현재 API에 맞췄고, `Tuple.to_bytes()` helper를 `storage/page/tuple.vais`에 추가해 WAL tuple payload builder의 호출부 drift를 제거했다. `dml.vais` 단독 codegen이 통과했고 최종 `check-integrity.sh`에서 fail list에서 제거되어 `vaisdb=244/261`로 상승했다.
- `src/graph/mod.vais`: graph facade의 `GraphMeta.deserialize`, `BufferPool.write_page`, graph lock, visibility helper, property index, WAL 호출부를 현재 하위 모듈 API에 맞췄다. `PropertyMap.entry_at` accessor를 추가해 serialized property backing store를 facade에서 안전하게 순회하게 했다. `graph/mod.vais` 단독 codegen이 통과했다.
- `src/rag/mod.vais`: `RagEngine.new(config, gcm)`로 WAL manager 생성 계약을 명확히 하고, hierarchy build, chunk graph edge plan, memory WAL, memory search score type, lifecycle action 호출부를 현재 RAG 하위 모듈 API에 맞췄다. `rag/mod.vais` 단독 codegen이 통과했고 최종 `check-integrity.sh`에서 `graph/mod.vais`, `rag/mod.vais`가 fail list에서 제거되어 `vaisdb=246/261`로 상승했다.
- `src/planner/pipeline.vais`: fulltext execution path가 현재 `CatalogManager`에 존재하지 않는 `get_fulltext_index`/metadata/posting-store accessor를 호출하던 ghost integration code를 제거했다. `ExecContext`에는 아직 fulltext engine/registry가 없으므로 `FULLTEXT_MATCH` planner node 인식은 유지하되 execution은 명시적인 deferred runtime error로 정리했다. `planner/pipeline.vais` 단독 codegen이 통과했고 최종 `check-integrity.sh`에서 fail list에서 제거되어 `vaisdb=247/261`로 상승했다.
- `src/sql/executor/join.vais`: `_impl_ptr` placeholder뿐이던 `RowSource`를 materialized `Vec<ExecutorRow>` adapter로 고정하고 `open(ctx)`, `next(ctx)`, `close()` 계약을 명시했다. `NestedLoopJoinExecutor`는 left/right source를 `open()`에서 materialize하고 cursor state로 CROSS/INNER/LEFT/RIGHT join을 순회하게 정리했다. 반쪽 구현이던 `USING` condition은 현재 타입체커 arm-local capture 제약 때문에 명시적 unsupported error로 고정했다. `join.vais` 단독 codegen이 통과했고 최종 `check-integrity.sh`에서 fail list에서 제거되어 `vaisdb=248/261`로 상승했다.
- `src/sql/executor/sort_agg.vais`: aggregate helper 반환 타입은 유지하고, `AggAccumulator` field pointer를 직접 갱신하던 코드를 accumulator enum slot 전체 재할당으로 바꿨다. `finalize_accumulator`는 각 match arm에서 명시적으로 `Result`를 반환하도록 고정했고, 오래된 `Integer(0)` fallback을 현재 `SqlValue.IntVal` variant로 정리했다. `sort_agg.vais` 단독 codegen이 통과했고 최종 `check-integrity.sh`에서 fail list에서 제거되어 `vaisdb=249/261`로 상승했다.
- `src/sql/executor/window.vais`: `partition_rows`의 `HashMap.get(... )!` 강제 unwrap을 `get_opt` match 기반 갱신으로 바꿔 totality contract를 지켰다. `HashMap` entry 직접 순회 대신 `values()` 순회를 사용하고, rank sort key를 `Option<Vec<SqlValue>>`/`Vec<SqlValue>`로 명시해 `prev_key[j]` indexability drift를 제거했다. `window.vais` 단독 codegen이 통과했고 최종 `check-integrity.sh`에서 fail list에서 제거되어 `vaisdb=250/261`로 상승했다.
- `src/sql/parser/parser_expr.vais`, `src/sql/parser/mod.vais`: parser expression helper에 typed `Vec<Expr>`/`Vec<OrderByItem>`/`Vec<WhenClause>` constructors를 추가하고, `TokenKind.Star`/`Expr.Star`처럼 token enum과 AST enum variant 이름이 겹치는 지점을 명시적으로 분리했다. `parser_expr.vais`, `parser/mod.vais`, `parser/token.vais` 단독 codegen이 통과했고 최종 `check-integrity.sh`에서 두 parser 파일이 fail list에서 제거되어 `vaisdb=252/261`로 상승했다.
- `src/storage/txn/conflict.vais`: `decode_lock_key()`가 `DecodedLockKey`를 반환하는데 `get_locks_for_txn()`이 tuple vector를 선언하던 contract mismatch를 named struct vector 반환으로 정리했다. `conflict.vais` 단독 codegen이 통과했고 최종 `check-integrity.sh`에서 fail list에서 제거되어 `vaisdb=253/261`로 상승했다.
- `src/storage/txn/deadlock.vais`: nested `HashMap<u64, Vec<u64>>` wait-for graph를 explicit `Vec<WaitEdge>` edge list로 바꿔 `Vec<u64>` inference drift를 제거했다. public wait-for graph API는 유지했고, DFS는 기존 구현이 실제로 쓰던 detected back-edge victim contract로 단순화했다. `deadlock.vais`와 이를 import하는 `txn/manager.vais` 단독 codegen이 통과했고 최종 `check-integrity.sh`에서 fail list에서 제거되어 `vaisdb=254/261`로 상승했다.
- `src/storage/btree/insert.vais`: `parent_guard`를 `Option<LatchGuard>`로 명시해 `Some(pg).release()` receiver type을 보존했고, old root header rewrite를 현재 `PageHeader.write_to_page_vec` 및 `BufferPool.get_page_mut` API로 맞췄다. `insert.vais` 단독 codegen이 통과했고 최종 `check-integrity.sh`에서 fail list에서 제거되어 `vaisdb=255/261`로 상승했다.
- `src/vector/hnsw/cow.vais`: `VaisError.new` 호출부를 현재 문자열 error-code contract로 맞추고, Rust식 atomic ordering 인자와 closure 기반 `ok_or_else`를 Vais stdlib API/명시 match로 정리했다. `Mutex.lock()!`/`get(...)!` 강제 unwrap은 totality-safe `lock()`/`get_opt` match로 바꿨고, layer mutation 뒤 `neighbors.set(...)`으로 값 기반 HashMap 갱신을 명확히 했다. `cow.vais` 단독 codegen이 통과했고 최종 `check-integrity.sh`에서 fail list에서 제거되어 `vaisdb=256/261`로 상승했다.
- `src/vector/hnsw/insert.vais`: 첫 삽입 여부를 존재하지 않는 `HnswMeta.len()`이 아니라 현재 `HnswMeta.is_empty()`로 판정하게 바꿨다. `hnsw_insert()`가 `&mut S` store를 들고 있으므로 내부 helper의 store 인자도 `&mut S`로 통일해 참조 contract drift를 제거했다. `insert.vais` 단독 codegen이 통과했고 최종 `check-integrity.sh`에서 fail list에서 제거되어 `vaisdb=257/261`로 상승했다.
- `src/vector/hnsw/delete.vais`, `src/vector/hnsw/types.vais`: `NodeStore` trait를 `insert.vais`에서 공통 `types.vais`로 이동해 insert/delete/WAL/storage가 같은 storage contract를 보게 했다. `hnsw_delete()`는 현재 `load_node`/`store_node` 계약으로 soft-delete node를 다시 저장하고, MVCC filtered search/GC scan은 read-only `NodeLookup` contract로 분리했다. `delete.vais`와 `insert.vais` 단독 codegen이 통과했고 최종 `check-integrity.sh`에서 fail list에서 제거되어 `vaisdb=258/261`로 상승했다.
- `src/vector/hnsw/wal.vais`: replay insert/update/promote를 현재 `NodeStore.load_node`/`store_node` 계약으로 바꾸고, remove API가 없는 replay delete/undo insert는 tombstone 저장 helper로 명시했다. vector data redo는 `BufferPool`의 frame-id 기반 API로 맞췄고, payload deserialize는 `ByteBuffer.wrap_readonly(payload_data)`를 사용하게 했다. numeric error code 호출도 문자열 code contract로 정리했다. `wal.vais` 단독 codegen이 통과했고 최종 `check-integrity.sh`에서 fail list에서 제거되어 `vaisdb=259/261`로 상승했다.
- `src/vector/hnsw/bulk.vais`, `src/vector/hnsw/insert.vais`: vector storage batch 단계의 잘못된 `meta.config.m` 스코프 참조를 caller가 넘긴 `m`으로 바꿨다. `hnsw_insert`와 insert helper들은 generic `S: NodeStore` 대신 공통 `&mut dyn NodeStore` contract를 직접 받도록 단순화했고, bulk loader도 같은 dyn trait contract를 사용한다. WAL-free bulk graph build는 아직 구현되지 않았으므로 `gcm!` 대신 명시적 error로 고정했다. `bulk.vais`와 `insert.vais` 단독 codegen이 통과했고 최종 `check-integrity.sh`에서 fail list에서 제거되어 `vaisdb=260/261`로 상승했다.
- `src/vector/mod.vais`, `src/vector/storage.vais`, `src/vector/hnsw/insert.vais`: vector facade가 현재 distance/HNSW/storage/WAL/filter/catalog 하위 API를 호출하도록 정리했다. 존재하지 않는 `pin_layer`, `log_insert`, `log_delete`, `log_bulk_load`, `fetch_row_by_tid` 호출은 제거했고, heap row materialization은 현재 catalog/executor surface가 생길 때까지 명시적 deferred error로 고정했다. `HnswNodeStore`에는 read-only `NodeLookup` 구현을 위한 vector cache를 추가했고, `insert.vais` 내부 priority queue helper는 `hnsw/search.vais`와 충돌하지 않도록 이름을 분리했다. `vector/mod.vais`, `vector/storage.vais`, `vector/hnsw/insert.vais`, `vector/hnsw/bulk.vais` 단독 codegen이 통과했고 최종 `check-integrity.sh`에서 `vaisdb=261/261`로 상승했다.
- Runtime/semantic smoke gate: `compiler/tests/vaisdb/smoke/vector_distance_smoke.vais`, `storage_bytes_smoke.vais`, `hnsw_node_smoke.vais`를 추가해 package import, binary link, 실행 exit code를 별도 검증한다. `vector/distance.vais`는 Result-returning 함수 안에서 enum match expression이 LLVM 반환형 추론을 흔들지 않도록 `distance_metric_tag` + 명시 branch로 고정했다. `vector/hnsw/types.vais`는 runtime 승격 중 드러난 `u8` match, nested Vec inference, indexed layer mutation 취약 패턴을 명시 source contract로 정리했고, HNSW node reference-return/nested Vec smoke는 compiler e2e gate로 승격됐다.
- Compiler Result payload invariant: `Result<T,E>`/`Option<T>` pattern binding이 enum layout용 `i64` 슬롯이 아니라 match expression의 concrete generic payload 타입을 사용하도록 고정했다. `vector_distance_smoke.vais`는 이제 `DistanceComputer.compute(&a, &b)`의 `Ok(v)`를 실제 `f32` 값으로 바인딩해 `-32.0`을 검증한다.
- Compiler HNSW runtime invariant: `Result<&Vec<T>,E>` pattern binding은 typed pointer payload로 복원되고, enum variant constructor는 reference payload를 layout slot에 `ptrtoint`로 저장한다. `&Vec<T>[i]`/`&mut Vec<T>[i]`는 backing storage element address를 반환하며, nested `Vec<Vec<HnswNeighbor>>` concrete instantiation은 typechecker registration/let binding 경로에서 보존된다. 같은 이름의 여러 `types.vais` module이 `.vais-cache/types.ll`을 공유하던 cache artifact collision은 path-hash suffix로 제거했다.
- SQL catalog schema runtime smoke: `compiler/tests/vaisdb/smoke/sql_catalog_schema_smoke.vais`를 추가해 `TableInfo`, `ColumnInfo`, `IndexInfo` serialize/deserialize와 catalog key bytes를 실행 시 검증한다. `src/sql/catalog/schema.vais`는 `IndexInfo.serialize`의 `Vec<str>` loop, `IndexInfo.deserialize` typed Vec 생성, catalog/security key helpers의 explicit `Vec<u8>` locals로 현재 compiler contract를 명확히 했다. compiler 쪽은 match default phi, match payload type inference, `Vec<str>` typed store, `&str` field refs, specialized `Option`/`Result` return coercion, auto-free alloc slot null initialization을 보강했다. `src/sql/types.vais` aggregate helpers는 owned clone/reference clone 경계를 명시했고, bool/null ordering match는 assignment 기반으로 정리했다. 최종 `check-integrity.sh` 요약은 `VAISDB RUNTIME OK: smoke=4/4`다.
- HNSW search runtime smoke: `compiler/tests/vaisdb/smoke/hnsw_search_smoke.vais`를 추가해 `NodeLookup` trait object, `Result<&HnswNode,E>`/`Result<&[f32],E>` payload, borrowed `Vec<f32>` slice, `SearchCandidate` heap ordering, `k=0` error path를 실행 시 검증한다. `src/vector/hnsw/search.vais`는 heap option returns와 peek contract를 명시했다. compiler 쪽은 dyn trait receiver/argument coercion, fat-slice payload packing/unwrapping, borrowed Vec slice return, `Vec.pop` explicit returns, struct/tuple ABI stride, recursive `alignof` guard를 보강했다. 최종 `check-integrity.sh` 요약은 `VAISDB RUNTIME OK: smoke=5/5`다.
- HNSW insert runtime smoke: `compiler/tests/vaisdb/smoke/hnsw_insert_smoke.vais`를 추가해 first/second insert, bidirectional neighbor edge, edge distance, missing-node `Err` path를 실행 시 검증한다. `src/vector/hnsw/insert.vais`는 `HashSet<u64>` visited set, typed `Vec<SearchCandidate>` heap, heap pop return contract를 명시했다. WAL writer/buffer/record_vector/recovery 경로는 `Vec<u8>` buffer, path string ownership, tuple payload serialization, checked read typing, `?`/match lowering 취약 패턴을 명시 source contract로 정리했다. compiler 쪽은 aggregate tuple field access, `HashSet` fallback layout, generic container layout, runtime sync/atomic helpers, `gettimeofday` declaration ownership, linkonce shallow-free helper emission을 보강했다. 현재 sync helper는 std/sync guard drop semantics 복원 전 deterministic single-thread shim이며, 후속 task에서 guard drop과 OS-level sync helper를 재승격해야 한다. 최종 `check-integrity.sh` 요약은 `VAISDB RUNTIME OK: smoke=6/6`다.
- SQL parser/planner runtime smoke: `compiler/tests/vaisdb/smoke/sql_parser_planner_smoke.vais`를 추가해 tokenizer byte input, keyword dispatch, parser select list/limit, AST `SelectItem.Expr`/`Expr.BinaryOp`, planner `PlanNode.Project`/`PlanNode.Limit`/`PlanNode.Values`를 실행 시 검증한다. `src/sql/parser/*.vais`는 `TokenKind.*`, `Expr.*`, `SelectItem.*`, `TableRef.*`, `JoinCondition.*`, `Statement.*`, `BinOp.*`처럼 중복 variant 이름을 명시했고, tokenizer keyword early-return/string byte cleanup, parser token tag helper, `SqlValue.in_list`/`agg_avg_accumulate` explicit total returns, `storage/wal/header.vais`의 `reserved: Vec<u8>` annotation으로 현재 source contract를 고정했다. compiler 쪽은 enum-name-scoped tag lookup, small named enum payload inline bitcast, Named let pointer-to-aggregate load, `Box<T>` handle/deref type, string `char_at`/`to_uppercase` helper emission, `Vec` complex specialization을 보강했다. 최종 `check-integrity.sh` 요약은 `VAISDB RUNTIME OK: smoke=7/7`다.
- SQL build_plan/CatalogManager runtime smoke: `compiler/tests/vaisdb/smoke/sql_build_plan_catalog_smoke.vais`를 추가해 실제 `CatalogManager`에 table/column/index metadata를 구성하고 `build_plan(&query, &catalog)`의 `Project -> Limit -> IndexScan` shape, cost row estimate, missing-table error path를 실행 시 검증한다. `src/sql/catalog/manager.vais`는 current compiler에서 불안정한 `HashMap<str, Struct>` cache를 `Vec` cache + index loop로 단순화했고, by-ref foreach getter를 index-based reference-return으로 바꿨다. `get_first_index_for_table` helper를 추가해 planner/smoke가 `Vec<&IndexInfo>` 반환 경로에 의존하지 않게 했다. `src/sql/parser/parser.vais`/`parser_select.vais`는 EOF check와 LIMIT/OFFSET parse를 borrowed enum/tuple nested Option 취약 경로에서 분리했다. `src/sql/planner/mod.vais`는 `Box.as_ref()`, `BinOp` tag helper, explicit limit extraction, conservative index selection, typed aggregate/order fallback으로 package codegen/runtime smoke를 안정화했다. storage/page/btree/recovery 경로는 explicit `Vec<u8>` annotation, slice copy helper, direct rwlock extern, Result/Option flow 정리로 `vaisdb_files=261/261`을 유지한다. compiler 쪽은 already-fat slice pass-through, `ByteBuffer.as_bytes()`, `Box<T>.as_ref()`, small-int generic return coercion을 보강했다. 최종 `check-integrity.sh` 요약은 `VAISDB RUNTIME OK: smoke=8/8`다. 이후 SQL planner precision task에서 aggregate/order/group extraction과 detailed index predicate usability를 runtime smoke로 승격했고, planner formatting fallback은 SQL planner format runtime smoke에서 제거됐다.
- HNSW delete runtime smoke: `compiler/tests/vaisdb/smoke/hnsw_delete_smoke.vais`를 추가해 `hnsw_delete()`의 soft-delete node 저장, affected layer list, `total_deleted`/`active_nodes` 갱신, tombstone metadata, duplicate delete error, invalid-id error를 실행 시 검증한다. smoke fixture는 delete module의 MVCC helper까지 link될 수 있도록 `NodeStore`와 `NodeLookup`을 모두 구현한다. `src/storage/txn/clog.vais`는 explicit `Vec<u8>`/`Vec<ClogPage>` locals와 `copy_clog_bytes()` helper로 slice copy/link drift를 제거했고, `src/vector/hnsw/delete.vais`는 affected layers `Vec<u8>`, node move 전 `max_layer` 보존, typed result vectors, `ceil_scaled_k()` helper로 runtime/codegen boundary를 고정했다. 최종 `check-integrity.sh` 요약은 `VAISDB RUNTIME OK: smoke=9/9`다.
- HNSW WAL runtime smoke: `compiler/tests/vaisdb/smoke/hnsw_wal_smoke.vais`를 추가해 HNSW WAL payload serialize/deserialize, redo insert/update/promote, undo insert/update/error paths, and `HnswWalManager.log_delete_node` LSN allocation을 실행 시 검증한다. `src/vector/hnsw/wal.vais`는 page-ref/u8/u64 slice copy helpers로 runtime link에서 불안정한 slice `.to_vec()` 의존을 제거했고, tuple payload와 nested/layer neighbor vectors를 명시 타입으로 고정했다. `dispatch_vector_redo/undo`는 `u8` record type을 `i64` tag로 명시 비교해 constant match lowering drift를 제거했다. 최종 `check-integrity.sh` 요약은 `VAISDB RUNTIME OK: smoke=10/10`다.
- HNSW bulk runtime smoke: `compiler/tests/vaisdb/smoke/hnsw_bulk_smoke.vais`를 추가해 `BulkLoadConfig` builder/validation, `BulkLoadProgress`, `BulkLoadResult`, `VectorMeta`, batch/memory helper, and WAL-free insertion error contract를 실행 시 검증한다. `src/vector/hnsw/bulk.vais`는 unstable dyn `NodeStore` graph build path와 unused imports를 제거하고, 당시 runtime-safe하지 않은 `bulk_insert()`를 explicit unimplemented error로 좁혔다. `src/vector/mod.vais`는 WAL-free bulk graph construction 미구현을 명시 error로 반환했다. `src/storage/page/overflow.vais`, `src/storage/page/freelist.vais`, `src/storage/page/allocator.vais`, `src/vector/storage.vais`의 unused/non-working helper와 inference drift도 함께 정리했다. 최종 `check-integrity.sh` 요약은 `VAISDB RUNTIME OK: smoke=11/11`다. 이후 real graph construction은 아래 HNSW bulk graph 재승격 항목에서 완료됐다.
- SQL planner precision runtime smoke: `compiler/tests/vaisdb/smoke/sql_planner_precision_smoke.vais`를 추가해 index predicate가 index column에만 적용되는지, non-index predicate는 SeqScan filter로 남는지, ORDER BY metadata와 GROUP BY/COUNT aggregate metadata가 보존되는지 실행 시 검증한다. `src/sql/planner/mod.vais`는 broad index fallback을 제거하고 predicate reference를 직접 검사하며, aggregate/order/group metadata를 빈 fallback으로 지우지 않게 했다. runtime smoke harness는 fixture별 source/temp dir과 dependency `.vais-cache` cleanup으로 ecosystem gate 직후에도 같은 결과가 나오도록 격리했다. 최종 `check-integrity.sh` 요약은 `VAISDB RUNTIME OK: smoke=12/12`다.
- SQL planner format runtime smoke: `compiler/tests/vaisdb/smoke/sql_planner_format_smoke.vais`를 추가해 `format_plan()`이 `"Plan"` fallback이 아니라 recursive exact string을 반환하는지 실행 시 검증한다. `src/sql/planner/mod.vais`는 `Project`, `Limit`, `IndexScan`, `Sort`, `Aggregate`, `SeqScan`, `Empty`, `Values` 등 plan node를 indent 포함 문자열로 포맷한다. compiler 쪽은 recursive `str` formatter가 드러낸 `&Str` alloca ref ABI, `Str` match phi null fallback, mutable string assignment ownership, block-scope string ownership transfer, `&Enum` duplicate unit variant tag lookup을 근본 수정했다. 최종 `check-integrity.sh` 요약은 `VAISDB RUNTIME OK: smoke=13/13`다.
- HNSW bulk graph 재승격: `src/vector/hnsw/bulk.vais`는 `bulk_insert_stored_vectors()`와 `insert_without_wal()` free function을 추가해 caller가 이미 저장한 vector page 위치, `NodeStore`, `DistanceComputer`, `HnswMeta`를 넘기면 WAL 없이 실제 HNSW search/select/prune/양방향 edge 구성을 수행한다. Associated method + `dyn NodeStore` fat pointer codegen 취약점은 숨기지 않고, 기존 `hnsw_insert`처럼 검증된 free-function trait-object 호출 계약으로 고정했다. `src/vector/mod.vais`의 `VectorEngine.bulk_load()`는 `skip_wal=true`에서 raw vector storage, node id/layer allocation, vector cache, graph build를 연결했다. 이 task 완료 시점에는 WAL-enabled bulk가 명시 unsupported error였지만, 아래 HNSW bulk WAL graph path에서 해제됐다. `compiler/tests/vaisdb/smoke/hnsw_bulk_smoke.vais`는 기존 error-path 검증 대신 two-vector real graph construction, bidirectional neighbor edge, edge distance, vector page metadata, no-WAL page allocation invariant를 실행 시 검증한다. 검증: `cargo fmt -p vaisc --check`, `cargo test -p vaisc --test e2e --release e2e_vaisdb_10_hnsw_bulk_runtime_smoke -- --nocapture`, `cargo test -p vaisc --test e2e --release e2e_vaisdb_vector_mod_facade_contract_compiles -- --nocapture`, `cargo test -p vaisc --test e2e --release e2e_vaisdb_ -- --nocapture`, `cd compiler && bash scripts/check-integrity.sh` 통과. 최종 요약은 `vaisdb=261/261`, `VAISDB RUNTIME OK: smoke=13/13`, `INTEGRITY OK`다.
- VectorEngine bulk end-to-end 재승격: `src/vector/storage.vais`의 `HnswNodeStore`가 `VectorEngine.new()` 지역 `VectorStore` 참조를 저장하던 dangling-reference 구조를 제거하고, 같은 `pool/allocator/bitmap`을 공유하는 owned `VectorStore` 필드로 고정했다. `compiler/std/bytebuffer.vais`는 read cursor와 write cursor를 분리해 `ByteBuffer.wrap(...); seek(...); write_*`가 실제 page buffer 위치를 갱신하면서도 serialize 직후 rewind 없이 deserialize하던 기존 catalog 패턴을 유지하게 했다. `compiler/tests/vaisdb/smoke/hnsw_search_smoke.vais`는 최신 `NodeStore` trait contract에 맞춰 fixture 구현을 보강했다. 검증: std ByteBuffer seek-write self-test `rc=0`, manual `vector_engine_bulk_smoke.vais` `rc=0`, `cargo test -p vaisc --test e2e --release phase_vaisdb_runtime_smoke:: -- --nocapture` 통과. 최종 요약은 `VAISDB RUNTIME OK: smoke=14/14`다.
- HNSW multi-layer/recall runtime smoke: `compiler/tests/vaisdb/smoke/hnsw_multilayer_recall_smoke.vais`를 추가해 explicit layer metadata를 가진 4-node WAL-free bulk graph가 max layer/entry point를 보존하고, upper/lower layer neighbor links를 구성하며, `knn_search()`가 layer descent 이후 query `(29.5, 0.0)`에서 node 4와 node 3을 거리 순서대로 반환하는지 실행 시 검증한다. 이는 bulk graph construction을 단순 compile/link가 아니라 multi-layer recall invariant로 고정한다. 검증: `cargo fmt`, `cargo test -p vaisc --test e2e --release e2e_vaisdb_17_hnsw_multilayer_recall_runtime_smoke -- --nocapture`, `cargo test -p vaisc --test e2e --release phase_vaisdb_runtime_smoke:: -- --nocapture` 통과. 최종 요약은 `VAISDB RUNTIME OK: smoke=15/15`다.
- HNSW bulk WAL graph path: `src/vector/hnsw/bulk.vais`에 `bulk_insert_stored_vectors_with_wal()`을 추가해 pre-stored vector metadata를 유지하면서 graph mutation은 기존 단일 `hnsw_insert()` WAL path로 통과시킨다. `src/vector/mod.vais`의 `VectorEngine.bulk_load()`는 `skip_wal=false`를 더 이상 unsupported error로 막지 않고 WAL graph path를 호출하며, 결과의 `wal_skipped` 값을 실제 config와 일치시킨다. `compiler/tests/vaisdb/smoke/hnsw_bulk_wal_smoke.vais`는 두 vector bulk insertion, HNSW insert WAL page allocation, bidirectional edge, vector page metadata 보존을 실행 시 검증한다. 이는 HNSW graph WAL 경로 재승격이며, raw vector page write WAL-before-mutation은 기존 single insert와 함께 별도 durability invariant로 남긴다. 검증: `cargo fmt`, `cargo test -p vaisc --test e2e --release e2e_vaisdb_18_hnsw_bulk_wal_runtime_smoke -- --nocapture`, `cargo test -p vaisc --test e2e --release phase_vaisdb_runtime_smoke:: -- --nocapture` 통과. 최종 요약은 `VAISDB RUNTIME OK: smoke=16/16`다.
- Inline raw vector page WAL-before-mutation: `src/vector/storage.vais`에 `VectorStore.write_vector_with_wal()`을 추가해 raw vector page image를 `VECTOR_DATA_WRITE` WAL record로 먼저 기록한 뒤 buffer frame을 mutate하도록 순서를 고정했다. `src/vector/mod.vais`의 single insert와 WAL-enabled bulk load는 이 API를 사용한다. `compiler/tests/vaisdb/smoke/vector_storage_wal_smoke.vais`는 실제 `VectorStore`/`BufferPool`/`WalWriter` 조합에서 WAL LSN이 생성되고 vector page readback이 보존되는지 실행 시 검증한다. overflow WAL은 아래 durability task에서 해제됐다. 검증: `cargo fmt`, `cargo test -p vaisc --test e2e --release e2e_vaisdb_19_vector_storage_wal_runtime_smoke -- --nocapture`, `cargo test -p vaisc --test e2e --release phase_vaisdb_runtime_smoke:: -- --nocapture` 통과. 최종 요약은 `VAISDB RUNTIME OK: smoke=17/17`다.
- Overflow vector WAL durability: `VectorStore.write_vector_with_wal()`이 overflow vector를 `VAIS-0208005`로 막지 않고, overflow chain page와 main overflow-pointer vector page의 full page image를 frame mutation 전에 `VECTOR_DATA_WRITE` WAL record로 기록한다. `VectorStore.read_vector()`는 overflow slot을 inline payload로 잘못 역직렬화하지 않고 main page의 `OverflowPointer`를 읽은 뒤 overflow chain을 `read_overflow_data()`로 복원해 원본 vector를 반환한다. overflow tuple 반환 literal은 `0 as u16`으로 명시해 slot payload ABI를 고정했다. 이 작업 중 `Result<(MvccTupleMeta, OverflowPointer), E>`처럼 struct-valued tuple element를 반환/field-access할 때 LLVM type이 깨지는 compiler tuple payload lowering 문제가 재현됐으므로, storage readback은 의미상 같은 순차 deserialize로 고정했고 compiler root fix는 다음 후보로 남긴다. `compiler/tests/vaisdb/smoke/vector_storage_overflow_wal_smoke.vais`는 2100차원 vector로 overflow chain을 강제하고 WAL LSN과 full readback을 실행 시 검증한다. 검증: `cargo test -p vaisc --test e2e --release e2e_vaisdb_20_vector_storage_overflow_wal_runtime_smoke -- --nocapture`, `cargo test -p vaisc --test e2e --release e2e_vaisdb_19_vector_storage_wal_runtime_smoke -- --nocapture`, `cargo test -p vaisc --test e2e --release phase_vaisdb_runtime_smoke:: -- --nocapture`, `cargo fmt --check`, `git diff --check` 통과. 최종 요약은 `VAISDB RUNTIME OK: smoke=18/18`다.
- Embedded durability re-entry: `src/server/embedded.vais`의 placeholder open/close를 실제 `storage/page/database.vais` create/open/close 경로에 연결했다. `src/storage/file.vais`는 `Database`가 기대하던 `Dir.create`/`Dir.remove_contents` directory helper를 명시 구현했고, `src/storage/page/bootstrap.vais`는 runtime codegen에서 Vec 로컬 타입이 약화되지 않도록 `Vec<u8>` annotation을 추가했다. `compiler/std/random.vais`의 `random_f64_simple()`도 `f64` 반환 계약에 맞게 명시 캐스팅으로 정리했다. `compiler/tests/vaisdb/smoke/embedded_durability_smoke.vais`는 `EmbeddedDatabase.open()`이 실제 `.vdb` 파일을 만들고, meta checkpoint LSN을 flush/close/reopen 뒤 보존하는지 실행 시 검증한다. 최종 runtime smoke 기준은 `VAISDB RUNTIME OK: smoke=28/28`이다.

다음 active task:
- 이 파일의 과거 후보인 compiler tuple payload lowering invariant, `VECTOR_DATA_WRITE` redo/recovery exact page-image replay, larger recall fixture는 root ROADMAP에서 이미 runtime smoke 또는 compiler invariant로 승격되어 완료됐다. Core freeze 이후 첫 VaisDB 작업인 embedded durability scenario도 `embedded_durability_smoke.vais`로 승격됐다. 다음 VaisDB 내부 작업은 root ROADMAP이 별도 downstream gate로 승격할 때만 진행한다.

---

## Superseded Phase Ω Notes (not active)

The Phase Ω task list below is historical. Do not resume Task #41, #42, or #43
from this file. Current execution is coordinated from
`/Users/sswoo/study/projects/vais/ROADMAP.md`, and vaisdb work remains blocked
until that roadmap promotes downstream package work again.

Useful historical data in this file may still explain old `vaisdb_files`
measurements, but it is not an active implementation plan.
